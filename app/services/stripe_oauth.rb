class StripeOauth < Struct.new(:user)
  ...
  def oauth_url(params)
    ...
    # redirecting the user there. This way we can handle errors (other than access_denied, which could come later).
    # See https://stripe.com/docs.connect/referece#get-authorize-errors
    begin
      response = RestClient.get url
      # If the request was successful, then we're all good to return this url.
  ...
  # Upon redirection back to this app, we will have a 'code' that we can use to get the access token and other details about our connected user.
  ...
  def deauthorize!
    ...
    desuthorized if response.code == 200 && user_id == user.stripe_user_id
  end

  # The actual deauthorization on our side consists of 'forgetting' the now-invalid user_id, API keys, etc.
  # Used here in #deauthorize! as well as in the webhook handler: app//controllers/hooks_controller.rb#stripe
  def deauthorized
  ...
  # A simple OAuth2 client we can use to generate a URL to redirect the user to as well as get an access token.
