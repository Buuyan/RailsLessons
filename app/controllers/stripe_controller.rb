class StripeController < ApplicationController
  ...
  # See app/services/stripe_oauth.rb for ##ouath_url details.
  ...
  def confirm
    connector = StripeOauth.new(current_user)
    if params[:code]
      # if we got a 'code' parameter. Then the connection was conpleted by the user.
      connector.verify!(params[:code])

    elsif params[:error]
      # If we have an 'error' parameter, it is because the user denied the connection request. Other errors are handled at #oauth_url generation time.
